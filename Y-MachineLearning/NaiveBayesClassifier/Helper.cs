...
namespace ProbabilityFunctions
{
    public static class Helper
    {
        ...
        {
            // By Alex :
            // If every value of an attribute is the same for a specific class, the standard_dev will be 0
            // and we will return NaN. To fix this in this special case, we will return 1 if we are on the 
            // mean and 0 if we are not. This means that this attribute has to be a specific value in for 
            // it to be classified as that class.
            if (standard_dev == 0)
            {
                if (x == mean)
                {
